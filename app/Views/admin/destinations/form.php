...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4">
        <div class="card-header">
            <i class="fas fa-<?= isset($destination) ? 'edit' : 'plus' ?> me-1"></i>
            Destination Form
        </div>
        <div class="card-body">
            <form action="<?= isset($destination) ? base_url('admin/destinations/update/' . $destination['id']) : base_url('admin/destinations/store') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="category" name="category" required>
                            ...
                            <?php foreach ($categories as $cat): ?>
                                <option value="<?= $cat ?>" <?= old('category', $destination['category'] ?? '') == $cat ? 'selected' : '' ?>>
                                    <?= ucfirst($cat) ?>
                                </option>
                            <?php endforeach ?>
                        </select>
                ...
                <div class="mb-3">
                    <label for="image" class="form-label">Image<?= !isset($destination) ? '*' : '' ?></label>
                    <input type="file" class="form-control" id="image" name="image" <?= !isset($destination) ? 'required' : '' ?>>
                    <small class="text-muted">Max size 2MB (JPEG, PNG, JPG)</small>
                    
                    <?php if (isset($destination) && $destination['image_url']): ?>
                        <div class="mt-2">
